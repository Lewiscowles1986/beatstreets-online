...
---
# JUDGE — 017 verdict: ACCEPT (user-facing artifacts fixed; knockback queued)

- The user's two reported symptoms map to two confirmed code defects + one
  aggressive debug default; all fixed with python-parity rationale.
- Carry-forward to 018: the hero knockback physics (the last blocker for the three
  weapon pixel gates): instrument per-frame hero vpos.x/vel.x around hits on both
  sides, diff the knockback constant + friction + hit cadence, fix, un-skip.
- Keep validating .github/workflows/fidelity.yml per push (green as of 016).
