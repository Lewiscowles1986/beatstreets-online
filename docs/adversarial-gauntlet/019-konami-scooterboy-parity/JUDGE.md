...
---
# JUDGE — 019 verdict: ACCEPT

- Both user-suspected issues confirmed and fixed: the cheat menu is now reachable
  (gated by 2 new e2e tests) and the scooterboy renders the ride/knock-off
  sequence per python (pinned by 6 new per-frame unit tests + visual captures).
- The 10 HARD gates stayed byte-identical through the input-path change (the
  edge-promotion semantics match the old keydown-capture timing exactly).
- Carry-forward to 020: the hero knockback physics (017's diagnosis) remains the
  last blocker for the three weapon pixel gates.
