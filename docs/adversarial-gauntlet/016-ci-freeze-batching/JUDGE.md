...
---
# JUDGE — 016 verdict: ACCEPT (CI stabilised with proof of zero frame drift)

- The fix targets the measured failure mode and is verified locally with identical
  frames and a 10x speedup; CI confirmation on the push is the closing evidence.
- Carry-forward: confirm the pushed main run goes green (the workflow's first real
  validation); keep weapon pixel gates skip-marked until the browser-vs-headless
  RNG trace lands; the local branch ci/fix-canvas-sizing-flake is now redundant
  (content rebased onto main as d1ac73f/15d2cbe) and can be deleted on request.
